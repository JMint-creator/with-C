...
const SettingItem = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex-1 flex items-center justify-between py-3.5 pr-4 ${!hideBorder ? "border-b border-[#E5E5EA]" : ""}`}
      >
        ...
        <div className="flex items-center gap-2">
          {isColor ? (
            ...
            !isTextarea && (
              <span className="text-[13px] text-[#888] truncate max-w-[120px]">
                {value}
              </span>
            )
          )}
...
const DecideView = ({
  ...
  onClose: () => void;
  themeConfig: any;
  onStartDecide: (delay: number, result: string[], tab: string) => void;
  isDeciding: boolean;
}) => {
  ...
  return (
    <div
      className="flex-1 min-h-[100dvh] w-full flex flex-col font-sans overflow-x-hidden relative"
      style={{ backgroundColor: themeConfig.bg }}
    >
      {/* Header */}
      <div
        className="w-full flex items-center justify-between px-4 pb-3 sticky top-0 z-10 pt-[max(1rem,env(safe-area-inset-top))]"
        style={{
          backgroundColor: themeConfig.bg ? themeConfig.bg + "cc" : "#fcfbf9cc",
          backdropFilter: "blur(12px)",
        }}
      >
        <button
          onClick={onClose}
          className="text-[#8e8e93] text-[15px] flex items-center active:opacity-50 transition-opacity w-[60px]"
        >
      ...
      <div className="w-full max-w-sm mx-auto px-5 pt-4 pb-20 flex-1 flex flex-col items-center">
        ...
        <div className="flex bg-[#e3e3e8] rounded-[10px] p-1 mb-8 w-full border border-black/[0.02]">
          <button
            className={`flex-1 py-1.5 rounded-[8px] text-[14px] font-medium transition-all ${tab === "tarot" ? "bg-white shadow-[0_1px_3px_rgba(0,0,0,0.1)] text-black" : "text-[#8e8e93]"}`}
            onClick={() => setTab("tarot")}
          >
            塔罗牌
          </button>
          <button
            className={`flex-1 py-1.5 rounded-[8px] text-[14px] font-medium transition-all ${tab === "yesno" ? "bg-white shadow-[0_1px_3px_rgba(0,0,0,0.1)] text-black" : "text-[#8e8e93]"}`}
            onClick={() => setTab("yesno")}
          >
            是/否
          </button>
          <button
            className={`flex-1 py-1.5 rounded-[8px] text-[14px] font-medium transition-all ${tab === "custom" ? "bg-white shadow-[0_1px_3px_rgba(0,0,0,0.1)] text-black" : "text-[#8e8e93]"}`}
        ...
        <div className="w-full bg-white rounded-[20px] p-5 shadow-sm border border-black/[0.04]">
            ...
            <div className="mb-5 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <span
                    className="text-[15px] font-medium w-4 text-center"
                    style={{ color: thumbColor }}
                  >
                    {tarotCount}
                  </span>
            ...
            <div className="mb-5 space-y-3">
              ...
              {options.length < 6 && (
                <button
                  ...
                  className="text-[#007AFF] text-[13px] font-medium flex items-center justify-center w-full py-2 bg-[#f2f2f7]/50 rounded-[10px]"
                >
          ...
          <div className="flex items-center justify-between mb-6 bg-[#fcfcfd] border border-gray-100 p-2.5 rounded-[12px] shadow-[0_2px_8px_-4px_rgba(0,0,0,0.05)]">
            <span
              className="text-[14px] font-medium pl-1 shrink-0"
              style={{ color: themeConfig.textPrimary }}
            >
              间隔
            </span>
            ...
            />
            <span
              className="text-[14px] font-medium min-w-[55px] text-right pr-1 shrink-0"
              style={{ color: thumbColor }}
            >
              {delay === 0 ? "立刻" : `${delay}分钟`}
            </span>
...
export default function App() {
  ...
  // Library States
  const [replySubTab, setReplySubTab] = useState<
    "cards" | "emoji" | "stickers" | "nudge"
  >("cards");
  ...
  const [nudges, setNudges] = useLocalState<string[]>("app_nudges", [
    "拍了拍我的 脑袋",
    "拍了拍我的 肩膀",
  ]);
  ...
  const [motto, setMotto] = useLocalState(
    "app_motto",
    "沉睡中缠绵 · 清醒又幻灭",
  );
  ...
  const [theme, setTheme] = useLocalState<
    "warm" | "mint" | "sakura" | "blue" | "purple" | "red"
  >("app_theme", "warm");
  ...
  const [chatBubbleColor, setChatBubbleColor] = useLocalState(
    "app_chatBubbleColor",
    "",
  );

  // Anniversary Settings
  const [anniversaryDate, setAnniversaryDate] = useLocalState(
    "app_anniversaryDate",
    "2024-10-28",
  );
  ...
  // Music Player State
  const [musicList, setMusicList] = useLocalState<
    Array<{ id: string; name: string; artist: string; url: string }>
  >("app_musicList", []);
  const [currentMusicIndex, setCurrentMusicIndex] = useLocalState<number>(
  ...
  const [globalDecideResult, setGlobalDecideResult] = useState<{
    tab: string;
    result: string[];
  } | null>(null);
      ...
      {showAddMusicModal && (
        <div className="fixed inset-0 z-[60] bg-black/40 flex items-center justify-center px-4 transition-opacity">
          <div className="bg-[#F2F2F7] rounded-[14px] w-full max-w-[270px] overflow-hidden flex flex-col pt-5">
            <span className="font-semibold text-[15px] text-black mb-4 text-center">
              添加音乐
            </span>
            <div className="px-4 flex flex-col gap-3 mb-5">
              ...
              />
              <span className="text-[10px] text-black/50 px-1 -mt-1 leading-tight">
                注意：必须是能够直接播放的原始音频链接。如果是普通网页或网盘分享链接将无法播放。
              </span>
            </div>
            <div className="flex w-full border-t border-[#3c3c43]/20">
              <button
                className="flex-1 py-3 text-[15px] text-[#007AFF] border-r border-[#3c3c43]/20 active:bg-black/5"
                onClick={() => setShowAddMusicModal(false)}
              >
                取消
              </button>
              <button
                className="flex-1 py-3 text-[15px] text-[#007AFF] font-semibold active:bg-black/5"
                onClick={() => {
      ...
      {confirmModal && (
        <div className="fixed inset-0 z-[60] bg-black/40 flex items-center justify-center px-4 transition-opacity">
          <div className="bg-[#F2F2F7] rounded-[14px] w-full max-w-[270px] overflow-hidden flex flex-col pt-5 items-center">
            <span className="font-semibold text-[15px] text-black mb-1 text-center">
              {confirmModal.title}
            </span>
            <span className="text-[11px] text-black/70 mb-5 px-4 text-center leading-tight">
              {confirmModal.msg}
            </span>
            <div className="flex w-full border-t border-[#3c3c43]/20">
              <button
              ...
              <button
                className="flex-1 py-3 text-[15px] text-[#FF3B30] font-semibold active:bg-black/5"
                onClick={() => {
      ...
      {globalDecideIsDrawing && globalDecideCountdown > 0 && (
        ...
        >
          <div className="w-8 h-8 rounded-full border border-gray-100 overflow-hidden mr-3 shrink-0">
            {chatAvatar2 ? (
              <img src={chatAvatar2} className="w-full h-full object-cover" />
            ) : (
              <div className="w-full h-full bg-[#f2f2f7]" />
            )}
          </div>
          <div className="flex flex-col pr-2">
            <span className="text-[12px] text-[#8e8e93] font-medium leading-tight">
              未婚夫思考中...
            </span>
            <span className="text-[15px] font-mono font-medium text-black leading-tight">
              {Math.floor(globalDecideCountdown / 60)
                .toString()
                .padStart(2, "0")}
              :{(globalDecideCountdown % 60).toString().padStart(2, "0")}
            </span>
          </div>
        </motion.div>
      )}
          ...
          {globalDecideResult.tab === "tarot" ? (
            ...
            >
                ...
                {globalDecideResult.result.map((r, i) => (
                  <motion.div
                    ...
                    style={{
                      fontSize: "80px",
                      color:
                        currentThemeConfig.numColor ||
                        currentThemeConfig.textPrimary,
                    }}
                  >
                    {r}
                  </motion.div>
              ...
              <button
                ...
                style={{
                  backgroundColor: currentThemeConfig.textPrimary,
                  color: currentThemeConfig.bg || "#fff",
                }}
              >
                确认
              </button>
            ...
            >
              ...
              <div className="text-[14px] text-[#8e8e93] mb-6 font-medium">
                未婚夫的答案是
              </div>
  ...
  if (view === "data") {
    return (
      <div
        className="flex-1 w-full bg-[#F2F2F7] flex flex-col min-h-[100dvh] overflow-x-hidden relative text-[12px]"
        style={{
          fontFamily:
      ...
      >
        ...
        >
          <button
            ...
            className="text-[#007AFF] text-[15px] flex items-center active:opacity-50 transition-opacity"
          >
        ...
        <div className="w-full max-w-md mx-auto px-4 pb-12 pt-6">
          <div className="mb-8">
            <div className="text-[11px] text-[#6d6d72] ml-4 mb-2 uppercase tracking-wide">
              本地数据
            </div>
  ...
  if (view === "library") {
    return (
      <div
        className="flex-1 w-full bg-[#F2F2F7] flex flex-col min-h-[100dvh] overflow-x-hidden relative text-[11px]"
        style={{
          fontFamily:
            '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif',
        }}
      >
        ...
        >
          <button
            onClick={() => setView("home")}
            className="text-[#007AFF] text-[14px] flex items-center active:opacity-50 transition-opacity w-[60px]"
          >
            <ChevronLeft size={24} className="-ml-1.5" />
            返回
          </button>
        ...
        <div className="flex-1 overflow-y-auto pb-12 w-full max-w-md mx-auto mt-2">
          <div>
            <div className="flex space-x-1 px-4 mt-2 font-medium text-[12px] border-b border-[#c6c6c8]/30">
              <button
                className={`px-3 py-2 ${replySubTab === "cards" ? "border-b-2 border-[#007AFF] text-[#007AFF]" : "text-[#8e8e93]"}`}
                onClick={() => setReplySubTab("cards")}
              >
                主字卡
              </button>
              <button
                className={`px-3 py-2 ${replySubTab === "emoji" ? "border-b-2 border-[#007AFF] text-[#007AFF]" : "text-[#8e8e93]"}`}
                onClick={() => setReplySubTab("emoji")}
              >
                Emoji
              </button>
              <button
                className={`px-3 py-2 ${replySubTab === "stickers" ? "border-b-2 border-[#007AFF] text-[#007AFF]" : "text-[#8e8e93]"}`}
                onClick={() => setReplySubTab("stickers")}
              >
                表情库
              </button>
              <button
                className={`px-3 py-2 ${replySubTab === "nudge" ? "border-b-2 border-[#007AFF] text-[#007AFF]" : "text-[#8e8e93]"}`}
                onClick={() => setReplySubTab("nudge")}
              >
                拍一拍
              </button>
            </div>
              ...
              <>
                {!activeGroupId ? (
                  <div className="mt-4 px-4 space-y-4 pb-8">
                    <div
                      className="flex justify-between items-center bg-white p-3 rounded-[10px] shadow-[0_1px_2px_rgba(0,0,0,0.05)] cursor-pointer active:bg-gray-50 transition-colors"
                      onClick={() => {
                    ...
                    <div className="space-y-3">
                      {cardGroups.map((group, groupIdx) => (
                        <div
                          key={group.id}
                          className="bg-white rounded-[10px] shadow-[0_1px_2px_rgba(0,0,0,0.05)] p-3 flex justify-between items-center cursor-pointer active:bg-gray-50 transition-colors"
                          onClick={() => setActiveGroupId(group.id)}
                        >
                          <div className="flex-1">
                            <div className="font-semibold text-[#000] text-[12px]">
                              {group.name}
                            </div>
                            <div className="text-[#8e8e93] text-[12px] mt-0.5">
                              {group.cards.length} 张字卡
                            </div>
                          </div>
                          <button
                            onClick={(e) => {
                            ...
                            className="text-[#c6c6c8] p-2 hover:text-[#FF3B30] active:text-[#FF3B30]"
                          >
                            <Trash2 size={16} />
                          </button>
                        </div>
                  ...
                  <div className="mt-2 px-4 pb-8">
                    <div className="flex items-center gap-2 mb-4 bg-white p-2 rounded-[10px] shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
                      <button
                        ...
                        className="p-1 active:opacity-50 text-[#007AFF]"
                      >
                        <ChevronLeft size={20} />
                      </button>
                      <input
                        ...
                        value={
                          cardGroups.find((g) => g.id === activeGroupId)
                            ?.name || ""
                        }
                        onChange={(e) => {
                          const newGroups = [...cardGroups];
                          const gIdx = newGroups.findIndex(
                    ...
                    <div className="space-y-2 max-h-[60vh] overflow-y-auto">
                      {(
                        ...
                        <div
                          key={idx}
                          className="flex justify-between items-center bg-white p-2.5 rounded-[10px] shadow-[0_1px_2px_rgba(0,0,0,0.05)]"
                        >
                          <textarea
                          ...
                          <button
                            ...
                            className="text-[#c6c6c8] active:text-[#FF3B30] ml-2 p-1 shrink-0"
                          >
                            <X size={16} />
                          </button>
                        </div>
              ...
              <div className="mt-4 px-4 space-y-2 pb-8">
                <div className="bg-white rounded-[10px] shadow-[0_1px_2px_rgba(0,0,0,0.05)] overflow-hidden">
                  {emojis.map((item, idx) => (
                    <div
                      key={idx}
                      className="flex justify-between items-center p-3 border-b border-[#E5E5EA] last:border-b-0"
                    >
                      <input
                      ...
                      <button
                        ...
                        className="text-[#c6c6c8] p-1 active:opacity-50"
                      >
                        <X size={18} />
                      </button>
                    </div>
              ...
              <div className="mt-4 px-4 pb-8">
                <div className="grid grid-cols-3 gap-3">
                  {stickers.map((url, idx) => (
                    <div
                      key={idx}
                      className="relative aspect-square bg-[#E5E5EA] rounded-[10px] overflow-hidden flex items-center justify-center group"
                    >
                      ...
                      <button
                        ...
                        className="absolute top-1.5 right-1.5 bg-black/50 text-white rounded-full p-1 active:opacity-50"
                      >
                        <X size={14} />
                      </button>
                    </div>
                  ...
                  <div
                    ...
                    className="aspect-square bg-white rounded-[10px] flex items-center justify-center cursor-pointer border border-dashed border-[#c6c6c8] shadow-[0_1px_2px_rgba(0,0,0,0.05)] active:bg-gray-50 transition-colors"
                  >
            ...
            {replySubTab === "nudge" && (
              <div className="mt-4 px-4 space-y-2 pb-8">
                ...
                <button
                  onClick={() => setNudges([...nudges, "拍了拍我的..."])}
                  className="w-full py-3 bg-white text-[#007AFF] font-medium rounded-[10px] shadow-[0_1px_2px_rgba(0,0,0,0.05)] flex items-center justify-center gap-2 mt-4 active:bg-gray-50 transition-colors"
                >
                  <Plus size={18} /> 添加拍一拍
                </button>
          ...
          <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center px-4">
            <div className="bg-white rounded-[14px] w-full max-w-sm overflow-hidden flex flex-col">
              ...
              <div className="max-h-[300px] overflow-y-auto">
                <div
                  className="p-3 border-b border-[#e5e5ea] flex justify-between items-center cursor-pointer active:bg-gray-50"
                  onClick={() => {
                    let newGroups = [...cardGroups];
                    let cardsToAdd: string[] = [];
                    if (Array.isArray(importModalData.data)) {
                      cardsToAdd = importModalData.data.filter(
                        (d) => typeof d === "string",
                      );
                      newGroups.push({
                        id: Date.now().toString(),
                        name: importModalData.name,
                        cards: cardsToAdd,
                      });
                    } else if (
                      importModalData.data &&
                      typeof importModalData.data === "object"
                    ) {
                      const newG = Object.keys(importModalData.data).map(
                        (k, i) => ({
                          id: Date.now().toString() + i,
                          name: k,
                          cards: Array.isArray(importModalData.data[k])
                            ? importModalData.data[k]
                            : [],
                        }),
                      );
                      newGroups.push(...newG);
                ...
                >
                  <span className="text-[#007AFF] text-[12px] font-medium">
                    新建分组
                  </span>
                </div>
                {cardGroups.map((group) => (
                  <div
                    key={group.id}
                    className="p-3 border-b border-[#e5e5ea] last:border-b-0 flex justify-between items-center cursor-pointer active:bg-gray-50"
                    onClick={() => {
                      let newGroups = [...cardGroups];
              ...
              <button
                className="p-3 text-[#FF3B30] font-medium text-[12px] border-t border-[#e5e5ea] active:bg-gray-50"
  ...
  if (view === "music_manager") {
    return (
      <div
        className="flex-1 w-full bg-[#F2F2F7] flex flex-col min-h-[100dvh] overflow-x-hidden relative text-[14px]"
        style={{
          fontFamily:
            '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif',
        }}
      >
        ...
        >
          ...
          <button
            ...
            className="text-[#007AFF] text-[15px] active:opacity-50 transition-opacity pr-2"
          >
            添加
          </button>
        </div>
          ...
          <div className="bg-white rounded-[10px] overflow-hidden">
            {musicList.length === 0 ? (
              ...
              musicList.map((music, idx) => (
                <div
                  key={music.id}
                  className={`flex items-center justify-between p-3 ${idx !== musicList.length - 1 ? "border-b border-[#c6c6c8]/30" : ""}`}
                >
                  ...
                  <button
                    onClick={() => {
                      ...
                      if (
                        currentMusicIndex >= newList.length &&
                        newList.length > 0
                      ) {
    ...
    return (
      ...
      >
        ...
        >
          <button
            onClick={() => setView("home")}
            className="text-[#007AFF] text-[14px] flex items-center active:opacity-50 transition-opacity"
          >
            <ChevronLeft size={24} className="-ml-1.5" />
            返回
          </button>
        ...
        <div className="w-full max-w-md mx-auto px-4 pb-12 pt-6">
          {/* 1. 界面美化 */}
          <div className="mb-8">
            <div className="text-[12px] text-[#6d6d72] ml-4 mb-2 uppercase tracking-wide">
              界面美化
            </div>
            <div className="bg-white rounded-[10px] overflow-hidden shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
              <SettingItem
                icon={ImageIcon}
                label="上传主壁纸"
                value={wallpaper ? "已上传 (重新上传)" : "未设置"}
                onClick={() => wallpaperInputRef.current?.click()}
              />
              <SettingItem
                icon={ImageIcon}
                label="上传顶部卡片图"
                value={profileBg ? "已上传" : "未设置"}
                onClick={() => profileBgInputRef.current?.click()}
              />
              <SettingItem
                icon={User}
                label="顶部头像 1"
                value={avatar1 ? "已上传" : "未设置"}
                onClick={() => avatar1InputRef.current?.click()}
              />
              <SettingItem
                icon={User}
                label="顶部头像 2"
                value={avatar2 ? "已上传" : "未设置"}
          ...
          <div className="mb-8">
            ...
            <div className="bg-white rounded-[10px] overflow-hidden shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
              ...
              <SettingItem
                icon={Users}
                label="我方聊天头像"
                value={chatAvatar1 ? "已上传" : "未设置"}
                onClick={() => chatAvatar1InputRef.current?.click()}
              />
              <SettingItem
                icon={Users}
                label="对方聊天头像"
                value={chatAvatar2 ? "已上传" : "未设置"}
                onClick={() => chatAvatar2InputRef.current?.click()}
              />
              <SettingItem
                icon={Droplet}
                label="聊天气泡 CSS"
                value={chatCss ? "已上传" : "未设置"}
                onClick={() => cssInputRef.current?.click()}
              />
              <SettingItem
                icon={Type}
                label="聊天字体 TTF"
                value={chatFont ? "已上传" : "未设置"}
        ...
        <input
          ...
          className="hidden"
          accept="image/*"
          onChange={(e) => handleFileChange(e, setChatBg)}
        />
        <input
          type="file"
        ...
        <input
          ...
          className="hidden"
          accept="image/*"
          onChange={(e) => handleFileChange(e, setProfileBg)}
        />
        <input
          type="file"
          ref={avatar1InputRef}
          className="hidden"
          accept="image/*"
          onChange={(e) => handleFileChange(e, setAvatar1)}
        />
        <input
          type="file"
          ref={avatar2InputRef}
          className="hidden"
          accept="image/*"
          onChange={(e) => handleFileChange(e, setAvatar2)}
        />

        <input
          type="file"
          ref={chatAvatar1InputRef}
          className="hidden"
          accept="image/*"
          onChange={(e) => handleFileChange(e, setChatAvatar1)}
        />
        <input
          type="file"
          ref={chatAvatar2InputRef}
          className="hidden"
          accept="image/*"
          onChange={(e) => handleFileChange(e, setChatAvatar2)}
        />
        ...
        <input
          ...
          accept=".css"
          onChange={(e) =>
            handleTextChange(e, setChatCss, "CSS已加载 (在主页面和聊天中生效)")
          }
        />
        <input
          type="file"
          ...
          onChange={(e) => handleFileChange(e, setChatFont)}
        />
        {renderOverlays()}
      </div>
    );
  ...
  return (
    ...
    >
      ...
      {chatFont && (
        <style
          dangerouslySetInnerHTML={{
            __html: `@font-face { font-family: 'CustomChatFont'; src: url('${chatFont}'); } * { font-family: 'CustomChatFont', sans-serif !important; }`,
          }}
        />
      )}
      ...
      >
        ...
        >
          ...
          >
            ...
            <div className="absolute -top-[36px] flex justify-center items-center w-full">
              <div className="relative flex items-center justify-center">
                <div
                  ...
                  style={{
                    borderColor: currentThemeConfig.bg,
                ...
                >
                  {avatar2 ? (
                    <img src={avatar2} className="w-full h-full object-cover" />
                  ) : (
                    <Cat size={30} strokeWidth={1.5} />
                  )}
                </div>
            ...
            <p
              className="text-[12px] leading-[1.6] text-center whitespace-pre-line"
              style={{ color: currentThemeConfig.textSecondary }}
            >
              {motto}
            </p>
        ...
        <div className="grid grid-cols-2 gap-3 shrink-0">
          ...
          >
            <div className="flex justify-between items-center mb-2">
              ...
              <button
                onClick={() => setView("music_manager")}
                className="w-6 h-6 rounded-full bg-black/5 flex items-center justify-center hover:bg-black/10 active:scale-95 transition-all"
              >
                <ListMusic
                  size={12}
                  style={{ color: currentThemeConfig.textSecondary }}
            ...
            <div className="w-full h-[3px] bg-black/5 rounded-full mb-4 relative mt-auto overflow-hidden">
              <div
                ...
                style={{
                  backgroundColor: currentThemeConfig.textSecondary,
            ...
            <div className="flex justify-between items-center px-1">
              <button
                onClick={prevMusic}
                className="w-7 h-7 rounded-full bg-black/[0.03] flex items-center justify-center transition-colors hover:bg-black/[0.06] active:scale-95"
                style={{ color: currentThemeConfig.textSecondary }}
              >
              ...
              <button
                ...
                className="w-8 h-8 rounded-full bg-black/[0.04] flex items-center justify-center transition-colors hover:bg-black/[0.07] active:scale-95"
                style={{ color: currentThemeConfig.textSecondary }}
              >
                {isMusicPlaying ? (
          ...
          >
            <div
              className="text-[12px] tracking-widest flex items-center justify-center font-medium"
              style={{ color: currentThemeConfig.textSecondary }}
            >
              <span>在一起已经</span>
            </div>
            ...
            <div
              className="text-[46px] leading-[1] italic font-normal tracking-tight my-2"
              style={{
                color: currentThemeConfig.textPrimary,
                fontFamily:
        ...
        >
          {tools.map((tool) => (
            ...
            >
              ...
              <span
                className="text-[11px] leading-none mt-0.5 font-medium"
                style={{ color: currentThemeConfig.textSecondary }}
              >
                {tool.name}
              </span>
      ...
      {chatKeepAlive && (
        <audio
          ...
          muted={
            false
          } /* This is an empty silent sample, playing it will keep iOS context active */
          playsInline
        />
      )}
    </div>
