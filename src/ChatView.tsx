...
export const ChatView = ({ onClose, onOpenSettings, themeConfig }: any) => {
  ...
  const [videoCallState, setVideoCallState] = useState<
    "none" | "calling" | "connected" | "incoming"
  >("none");
      ...
      timer = setTimeout(
        () => {
          if (Math.random() < 0.25 && videoCallState === "none") {
            ...
            const pushNotify =
              window.localStorage.getItem("app_chatPushNotify");
            if (
              (pushNotify ? JSON.parse(pushNotify) : true) &&
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < replyCount; i++) {
        ...
        if (replyCards.length > 0) {
          content =
            replyCards[Math.floor(Math.random() * replyCards.length)].content;
        }
        ...
        if (Math.random() < 0.3) {
          ...
          if (recentMyMsgs.length > 0) {
            replyToMsg =
              recentMyMsgs[Math.floor(Math.random() * recentMyMsgs.length)]
                .content;
          }
        }
  ...
  const customStyle = chatFont
    ? { fontFamily: "CustomChatFont, sans-serif" }
    : {};
  ...
  return (
    ...
    >
      {chatFont && (
        <style
          dangerouslySetInnerHTML={{
            __html: `@font-face { font-family: 'CustomChatFont'; src: url('${chatFont}'); }`,
          }}
        />
      )}
      ...
      {/* Header */}
      <div className="shrink-0 pt-[max(0.5rem,env(safe-area-inset-top))] px-3 pb-2 w-full z-20 relative">
        <div
          className="rounded-[12px] flex items-center justify-between px-3 py-2 shadow-[0_4px_20px_rgba(0,0,0,0.05)] border border-white/50"
          style={{
        ...
        >
          <button
            onClick={onClose}
            className="w-[70px] flex items-center p-1 text-black/60 active:opacity-50 transition-opacity"
          >
            <ChevronLeft size={26} className="-ml-1" />
          </button>
          <div className="flex-1 flex items-center justify-center space-x-2 relative min-h-[38px]">
            {avatar2 && (
              <div className="w-[42px] h-[42px] rounded-full overflow-hidden shadow-sm border border-black/5 shrink-0 bg-white">
                <img
                  src={avatar2}
                  alt=""
            ...
            <input
              ...
              className="text-[15px] font-serif font-medium text-center bg-transparent outline-none m-0 p-0 text-black leading-none tracking-wide"
          ...
          <div className="w-[70px] flex items-center justify-end pr-1">
            <button
              ...
              className="text-black/60 active:opacity-50 transition-opacity p-1.5"
            >
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto px-4 pt-2 pb-4 flex flex-col scrollbar-hide z-10 relative w-full"
      >
        {messages.map((msg, i) => {
          ...
          const isRead =
            messages.slice(i + 1).some((m) => m.sender === "them") ||
            (readNoReply && !isTyping);

          if (msg.type === "nudge") {
            return (
              <div
                key={msg.id}
                className={`w-full flex justify-center ${marginBottom}`}
              >
            ...
            return (
              ...
              >
                <div className="px-4 py-2 rounded-2xl bg-black/[0.03] text-[13px] flex items-center space-x-2 text-gray-600">
                  {msg.callState === "declined" ||
                  msg.callState === "missed" ? (
          ...
          return (
            <div
              key={msg.id}
              className={`flex w-full ${isMe ? "justify-end" : "justify-start"} ${marginBottom}`}
            >
              {!isMe && (
                <div className="w-9 h-9 shrink-0 mr-3">
                  {!isGroupedNext && (
                    <div className="w-full h-full rounded-full bg-black/10 overflow-hidden shadow-sm">
                      {avatar ? (
                        <img
                          src={avatar}
                          alt=""
              ...
              >
                {msg.replyTo && (
                  <div
                    className={`text-[12px] px-3 py-1.5 mb-1 rounded-[12px] opacity-70 truncate max-w-full ${isMe ? "bg-black/10 text-black" : "bg-white/50 text-black/60"} border border-black/5`}
                  >
                    回复: {msg.replyTo}
                  </div>
                )}
                ...
                {!isGroupedNext && (
                  <div
                    className={`text-[10px] mt-1 flex items-center space-x-1 ${isMe ? "text-black/30" : "text-black/20"}`}
                  >
                    <span>{msg.time}</span>
                    {isMe && readReceipt && (
                      <span className="ml-1 flex items-center">
                        {receiptStyle === "text" ? (
                          <span
                            style={{ color: isRead ? bubbleColor : "inherit" }}
                            className={isRead ? "font-medium" : ""}
                          >
                            {isRead ? "已读" : "送达"}
                          </span>
                        ) : isRead ? (
                          <CheckCheck
                            size={18}
                            strokeWidth={2.5}
                            style={{ color: bubbleColor, marginLeft: "2px" }}
                          />
                        ) : (
                          <Check
                            size={16}
      ...
      <div className="shrink-0 relative z-30 px-3 pb-[max(0.5rem,env(safe-area-inset-bottom))] mt-1 w-full">
        {/* Plus Menu Overlay */}
        <AnimatePresence>
          {showPlusMenu && (
            <motion.div
              ...
              className="absolute bottom-[100%] mb-2 left-3 right-3 bg-white/70 backdrop-blur-xl rounded-[16px] py-4 flex justify-around px-2 items-center shadow-[0_10px_40px_rgba(0,0,0,0.08)] border border-white/50"
            >
              <button
                className="flex flex-col items-center gap-2 group w-[64px]"
                onClick={() => {
                  initiateCall();
                  setShowPlusMenu(false);
                }}
              >
                <div
                  className="w-[50px] h-[50px] rounded-[16px] flex items-center justify-center group-active:scale-95 transition-transform"
                  style={{
              ...
              <div
                className="flex flex-col items-center gap-2 group cursor-pointer w-[64px]"
        ...
        <div className="rounded-[16px] bg-white/70 backdrop-blur-xl border border-white/80 p-2 flex flex-col shadow-[0_8px_30px_rgba(0,0,0,0.06)]">
          <div className="flex items-center space-x-1.5">
            <button
              className={`w-9 h-9 flex items-center justify-center shrink-0 ${showPlusMenu ? "rotate-45" : ""} transition-all active:scale-95`}
            ...
            <div
              className="flex-1 min-h-[32px] rounded-[16px] flex items-center px-3 py-1 transition-colors border border-black/[0.03]"
            ...
            {input.trim() ? (
              <button
                className="w-9 h-9 flex items-center justify-center shrink-0 text-white rounded-full shadow-md active:scale-95 transition-transform"
            ...
            <div className="absolute inset-0 flex flex-col">
              {!isMinimized && (
                <div className="flex items-center justify-between p-6 pt-12">
                  ...
                  >
                    <ChevronLeft size={28} className="-ml-2 rotate-[-90deg]" />
                  </button>
              ...
              <div
                className={`flex-1 flex flex-col items-center ${isMinimized ? "justify-center p-2" : "justify-start pt-16"}`}
              >
                {/* Avatar */}
                <div
                  className={`${isMinimized ? "w-12 h-12" : "w-24 h-24"} rounded-full bg-white/10 overflow-hidden mb-4 shadow-lg border-2 border-white/10`}
                >
                  {avatar2 ? (
                    <img
                      src={avatar2}
                ...
                {!isMinimized && (
                  <div className="text-[28px] font-medium text-white mb-2">
                    {charId}
                  </div>
                )}

                <div
                  className={`${isMinimized ? "text-[10px]" : "text-[15px]"} text-white/50 tracking-widest`}
                >
              ...
              {!isMinimized && (
                <div className="pb-16 px-10 flex items-center justify-center space-x-8">
                  {videoCallState === "incoming" ? (
                    <>
                      <button
                        className="w-16 h-16 rounded-full bg-red-500 flex items-center justify-center text-white shadow-[0_4px_20px_rgba(239,68,68,0.4)] active:bg-red-600"
                        onClick={declineCall}
                      >
                        <Phone size={28} className="rotate-[135deg]" />
                      </button>
                      <button
                        className="w-16 h-16 rounded-full bg-green-500 flex items-center justify-center text-white shadow-[0_4px_20px_rgba(34,197,94,0.4)] active:bg-green-600"
                        onClick={acceptCall}
                      >
                        <Phone size={28} />
                      </button>
                    ...
                    <>
                      <button className="w-14 h-14 rounded-full bg-white/10 flex items-center justify-center text-white/80 active:bg-white/20">
                        <MicOff size={24} />
                      </button>
                      <button
                        className="w-16 h-16 rounded-full bg-red-500 flex items-center justify-center text-white shadow-[0_4px_20px_rgba(239,68,68,0.4)] active:bg-red-600"
