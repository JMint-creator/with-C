...
old_handle_purchase = """  const handlePurchase = (gameName: string, cost: number) => {
...
new_handle_purchase = """  const handlePurchase = (gameName: string, cost: number, onSuccess?: () => void) => {
    if (virtualBalance < cost) {
      showToast(`余额不足，还需要 ${cost - virtualBalance} 元`);
      return;
    }
...
if old_handle_purchase in content:
    content = content.replace(old_handle_purchase, new_handle_purchase)
else:
    print("WARNING: Could not find old_handle_purchase")


games_list_start = """        {/* Games List */}
...
# replace games list
start_idx = content.find(games_list_start)
end_idx = content.find(games_list_end)

if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + new_games_list + content[end_idx:]
    with open('src/GameCenterView.tsx', 'w') as f:
        f.write(content)
    print("Games list updated.")
else:
    print("Could not find games list bounds.")
